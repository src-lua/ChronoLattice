...
class Config:
    INPUT_FILE = os.path.join(BASE_DIR, "R37_tarefas_4.txt")
...
def make_hist_signature(
    ...
    # eventos por recurso: +1 em st, -1 em ft
...
class ProjectData:
    ...
    def _calculate_base_duration(self):
        ...
        if not base_starts: raise ValueError("O cronograma base é inválido ou cíclico.")
        self.base_duration = max((base_starts[tid] + self.durations[tid] for tid in self.task_ids), default=0)
        self.max_duration_allowed = int(math.ceil(self.base_duration * self.config.MAX_DURATION_MULTIPLIER))

...
class GanttPlotter:
    ...
    def _plot_resource_histogram(self, ax, gantt_data, base_tasks_map, resources_to_plot, resource_color_map):
        """Cria o Histograma de Uso de Recursos com limitação inteligente de ticks no eixo X."""
        if not base_tasks_map or not resources_to_plot:
            return
        ...
        # Matriz de uso por recurso ao longo do tempo
        resource_usage = {res: [0] * makespan for res in resources_to_plot}

        for task in gantt_data:
            base_info = base_tasks_map.get(task['Task'])
            if not base_info or len(base_info) < 6:
                continue
            resource_id = base_info[4]
            resource_qty = int(base_info[5])

            if resource_id not in resource_usage:
                continue

            # Soma do uso do recurso no intervalo [Start, Finish)
            start, finish = int(task['Start']), int(task['Finish'])
            for t in range(start, min(finish, makespan)):
                if 0 <= t < makespan:
                    resource_usage[resource_id][t] += resource_qty

        time_axis = list(range(makespan))
        bottom = [0] * makespan

        for res in resources_to_plot:
            usage = resource_usage.get(res, [0] * makespan)
            ax.bar(
                time_axis, usage,
                bottom=bottom,
                width=1.0, align='edge',
                edgecolor='black', linewidth=0.2,
                color=resource_color_map.get(res),
                label=str(res)
            )
            # acumula para o próximo recurso (empilhamento)
            bottom = [b + u for b, u in zip(bottom, usage)]

        ax.set_xlabel("Tempo")
        ax.set_ylabel("Uso de Recursos")
        ax.legend(loc="upper right", fontsize="small", ncol=2)

        # Limita o número de marcas no eixo X para evitar explosão de labels
...
class ScheduleDataManager:
    ...
    def load_schedules_list_stream(self, file_path):
        """Lê a lista de cronogramas linha a linha (streaming), evitando carregar 200MB em uma única string."""
        parsed_schedules = {}
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                for i, line in enumerate(f, 1):
                    ...
                    try:
                        ...
                        messagebox.showerror("Erro de Leitura", f"Erro de sintaxe na linha {i}: {e}")
                        return False

                    if isinstance(item, list) and len(item) > 1:
                        parsed_schedules[str(item[0])] = item[1:]
                    else:
                        # Mantém o processamento (apenas avisa)
                        messagebox.showwarning("Formato Inesperado", f"A linha {i} tem um formato inválido.")

            # preserva apenas a base e mescla os cronogramas lidos
            self.all_schedules = {k: v for k, v in self.all_schedules.items() if k == BASE_SCHEDULE_ID}
            self.all_schedules.update(parsed_schedules)
            return True

        except Exception as e:
            messagebox.showerror("Erro de Leitura", f"Falha ao processar a lista: {e}")
            return False
    ...
    def calculate_gantt_data(self, tasks):
        """Calcula os tempos de início e fim para cada tarefa, usando deque para O(1) nas retiradas."""
        if not tasks:
            return []

        # Import local para não depender de alteração no cabeçalho do arquivo
        ...
        while queue:
            ...
            duration = int(task_info[1])
            pred1_str = task_info[2]
            pred2_str = task_info[3]

            # Para início mais cedo, consideramos TODAS as predecessoras (obrigatórias e arbitrárias)
            all_preds = [p.strip() for p in pred1_str.split(',') if p.strip()] + \
                        [p.strip() for p in pred2_str.split(',') if p.strip()]

            earliest_start = 0
            for pred_id in all_preds:
                # Se a predecessora não existe/foi filtrada, ignoramos silenciosamente
                if pred_id in finish_times:
                    if finish_times[pred_id] > earliest_start:
                        earliest_start = finish_times[pred_id]

            start_times[current_task_id] = earliest_start
            finish_times[current_task_id] = earliest_start + duration

            gantt_chart_data.append({
                'Task': current_task_id,
                'Start': start_times[current_task_id],
                'Finish': finish_times[current_task_id],
                'Duration': duration
            })

            for succ_id in dependencies.get(current_task_id, []):
                in_degree[succ_id] -= 1
        ...
        if processed != len(tasks):
            # Ciclo ou alguma inconsistência no grafo
            raise ValueError("Ciclo detectado nas dependências ou tarefas ausentes.")

        return gantt_chart_data

    def get_annotated_gantt_data(self, schedule_id):
            """
            Retorna (gantt_data, raw_tasks) para o 'schedule_id' selecionado,
            agora com os dados de Gantt enriquecidos com informações de camada e rede.
            - gantt_data: lista de dicts com 'Task', 'Start', 'Finish', 'Duration',
                        'Layer', 'NetworkID', 'NetworkDuration'.
            - raw_tasks: lista crua de tarefas do cronograma.
            """
            if not hasattr(self, "all_schedules") or not isinstance(self.all_schedules, dict):
                raise RuntimeError("Estrutura 'all_schedules' não inicializada.")

            tasks = self.all_schedules.get(schedule_id)
            if not tasks:
                return [], []

            gantt_data = self.calculate_gantt_data(tasks)

            # Anota os dados do Gantt com informações de camada e rede para a ordenação
            for task_dict in gantt_data:
                task_id = task_dict['Task']
                task_dict['Layer'] = self.task_layers_map.get(task_id, float('inf'))
                network_id = self.task_network_map.get(task_id)
                task_dict['NetworkID'] = network_id
                if network_id:
                    task_dict['NetworkDuration'] = self.network_durations.get(network_id, 0)
                else:
                    task_dict['NetworkDuration'] = 0
...
class GanttApp:
    ...
    def _create_filter_options_frame(self, parent):
        ...
        param_keys = ["folga", "linearidade", "expansao", "regularidade"]
    ...
    def _load_schedules_file(self):
        """Abre o arquivo de LISTA de cronogramas e carrega em streaming (linha a linha)."""
        file_path = filedialog.askopenfilename(
            title="Selecione a LISTA de cronogramas",
            filetypes=[("Arquivos de Texto", "*.txt")]
        )
        if not file_path:
            return

        if self.data_manager.load_schedules_list_stream(file_path):
            self.schedules_file_label.config(text=os.path.basename(file_path), foreground="green")
            self.root.title(f"Visualizador de Cronogramas - {os.path.basename(file_path)}")
            self._update_ui_after_load()
    ...
    def _update_ui_after_load(self):
        """Atualiza a UI após carregamento, calculando os makespans em background para não congelar o Tkinter."""
        if not self.data_manager.all_schedules:
            return

        import threading

        def worker():
            # Cálculo pesado em thread separada
            self.data_manager.calculate_all_makespans()
            sorted_durations = sorted(self.data_manager.durations_map.keys())

            # Atualização de UI deve ocorrer no main thread (Tkinter)
            def apply_results():
    ...
    def _update_cascading_filters(self, event=None):
        ...
        def update_combobox_options(combobox, data_key):
            ...
            active_filters['duracao'] = (data_key != 'duracao')
        ...
        # --- Atualização da UI ---
        update_combobox_options(self.duration_combobox, 'duracao')
        ...
        final_active_filters['duracao'] = True
